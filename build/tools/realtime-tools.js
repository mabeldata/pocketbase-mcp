...
export function listRealtimeTools() {
    return realtimeToolInfo;
}
// Handle calls for realtime-related tools
export async function handleRealtimeToolCall(name, args, pb) {
    switch (name) {
        case 'subscribe_collection':
            return subscribeCollection(args, pb);
        case 'unsubscribe_collection':
            return unsubscribeCollection(args, pb);
        default:
            throw new Error(`Unknown realtime tool: ${name}`);
    }
}
// --- Individual Tool Implementations ---
async function subscribeCollection(args, pb) {
...
async function unsubscribeCollection(args, pb) {
