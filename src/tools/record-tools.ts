...
async function deleteRecord(args: DeleteRecordArgs, pb: PocketBase): Promise<ToolResult> {
    ...
    // SDK delete() resolves true when the server confirmed the removal (204).
    const deleted = await pb.collection(args.collection).delete(args.id);
    return {
        content: [{
            type: 'text',
            text: JSON.stringify({ deleted: !!deleted, collection: args.collection, id: args.id }, null, 2),
        }],
    };
}
